...
test_single() {
    ...
    llvm-link-17 --opaque-pointers $func_testcase_dir/$test_name.ll sylib.ll -S -o ./output/$test_name.ll
	if [ $? != 0 ]; then
		echo "fail to link"; exit -1
	fi
	if [ -f $func_testcase_dir/$test_name.in ]; then
    	lli-17 --opaque-pointers ./output/$test_name.ll < $func_testcase_dir/$test_name.in > output/$test_name.out
	else
    	lli-17 --opaque-pointers ./output/$test_name.ll > ./output/$test_name.out
	fi
